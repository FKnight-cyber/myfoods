...
import __categoryFactory from "../factories/categoryFactory";
...
describe("POST /categories/create", () => {
    it("should return status 201 when successfully create a new category", async () => {
        const category = await __categoryFactory();
        ...
        const result = await supertest(app).post("/categories/create").send(category).set('x-access-token', token);
    ...
    it("should return status 409 when tries to create duplicated category", async () => {
        ...
        await supertest(app).post("/categories/create").send(category).set('x-access-token', token);
...
describe("GET /categories", () => {
    it("should return status 200 and all registered categories", async () => {
        const category1 = await __categoryFactory();
        const category2 = await __categoryFactory();
        const admin = await __adminFactory();
        ...
        await supertest(app).post("/categories/create").send(category1).set('x-access-token', token);
        await supertest(app).post("/categories/create").send(category2).set('x-access-token', token);

        const result = await supertest(app).get("/categories").send().set('x-access-token', token);
...
describe("PATCH /categories/:id", () => {
    it("should return status 401 when unauthorized user tries to edit a category", async () => {
        ...
        const result = await supertest(app).patch("/categories/1").send({name:"edit"}).set('x-access-token', token);
    ...
    it("should return status 404 when tries to edit unregistered category", async () => {
        ...
        const result = await supertest(app).patch("/categories/99999").send({name:"edit"}).set('x-access-token', token);
    ...
    it("should return status 200 when successfully edit a category", async () => {
        ...
        const checkCategory = await prisma.category.findUnique({where:{name:category.name}});

        const result = await supertest(app).patch(`/categories/${checkCategory.id}`).send({name:"edit"}).set('x-access-token', token);
...
describe("DELETE /categories/delete/:id", () => {
    it("should return status 401 when unauthorized user tries to delete a category", async () => {
        ...
        const result = await supertest(app).delete("/categories/delete/1").send().set('x-access-token', token);
    ...
    it("should return status 404 when tries to delete unregistered category", async () => {
        ...
        const result = await supertest(app).delete("/categories/delete/99999").send().set('x-access-token', token);
    ...
    it("should return status 200 when successfully remove a category", async () => {
        ...
        const result = await  supertest(app).delete(`/categories/delete/${checkCategory.id}`).set('x-access-token', token);

        const shouldBeEmpty = await prisma.category.findUnique({where:{name:category.name}});
